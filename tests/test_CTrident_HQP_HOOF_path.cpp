...
//    vs::CAdaptiveGain lambda_vx(1.666, 0.166, 1.666);
    ...
    //Get desired/goal image / initial waypoints
    d_img = cv::imread("goal.jpeg");
        ...
        if(c_img.empty()==false)
        {
            ...
            if(nMatches > 3)
            {
                ...
                */
                ...
                B1.at<double>(5,0) = -yaw_gain * e_w.at<double>(2);
...
/*
                ...
                double err_thresh_0 = 10.0;
                double err_thresh_1 = 10.0;
                double err_thresh_2 = 0.1;
                double err_thresh_3 = 5.0;

                if((std::abs(e_v.at<double>(0)) < err_thresh_0) && (std::abs(e_v.at<double>(1)) < err_thresh_1)
                    && (std::abs(log(rho2)) < err_thresh_2) && (std::abs(e_w.at<double>(2)) < err_thresh_3))

*/
                // Speed-based convergence criteria
                double speed = vs::utils::vectorL2Norm(solution);
...

                {
                    ...
                    {
                        std::cout << "[+] Moving to waypoints 2" << std::endl;
                        d_img = cv::imread("target_2.jpeg");
                    }

                    if(currentWaypoint == 3)
                    {
                        std::cout << "[+] Moving to waypoints 3" << std::endl;
                        d_img = cv::imread("target_3.jpeg");
                    }

                    if(currentWaypoint == 4)
                    {
                        std::cout << "[+] Moving to waypoints 4" << std::endl;
                        d_img = cv::imread("target_4.jpeg");
                    }

                    if(currentWaypoint == 5)
                    {
                        std::cout << "[+] Moving to waypoints 5" << std::endl;
                        d_img = cv::imread("target_5.jpeg");
                    }

                    if(currentWaypoint == 6)
                    {
                        std::cout << "[+] Moving to waypoints 6" << std::endl;
                        d_img = cv::imread("target_6.jpeg");
                    }
                    ...
                    {
                        std::cout << "[+] Moving to waypoints 7" << std::endl;
                        cv::Mat temp = cv::imread("target_7.jpeg");
                        rotate(temp, 45.0, d_img);
                    }
                    ...
                    {
                        std::cout << "[+] Moving to waypoints 8" << std::endl;
                        cv::Mat temp = cv::imread("target_8.jpeg");
                        rotate(temp, 90.0, d_img);
                    }

                    // Change direction
                    if(currentWaypoint == 9)
                    {
                        std::cout << "[+] Moving to waypoints 9" << std::endl;
                        cv::Mat temp = cv::imread("target_9.jpeg");
                        rotate(temp, 135.0, d_img);
                    }

                    if(currentWaypoint == 10)
                    {
                        std::cout << "[+] Moving to waypoints 10" << std::endl;
                        cv::Mat temp = cv::imread("target_10.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 11)
                    {
                        std::cout << "[+] Moving to waypoints 11" << std::endl;
                        cv::Mat temp = cv::imread("target_11.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 12)
                    {
                        std::cout << "[+] Moving to waypoints 12" << std::endl;
                        cv::Mat temp = cv::imread("target_12.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 13)
                    {
                        std::cout << "[+] Moving to waypoints 13" << std::endl;
                        cv::Mat temp = cv::imread("target_13.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 14)
                    {
                        std::cout << "[+] Moving to waypoints 14" << std::endl;
                        cv::Mat temp = cv::imread("target_14.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 15)
                    {
                        std::cout << "[+] Moving to waypoints 15" << std::endl;
                        cv::Mat temp = cv::imread("target_15.jpeg");
                        rotate(temp, 180.0, d_img);
                    }

                    if(currentWaypoint == 16)
                    {
                        std::cout << "[+] Moving to waypoints 16" << std::endl;
                        cv::Mat temp = cv::imread("target_16.jpeg");
                        rotate(temp, 225.0, d_img);
                    }

                    if(currentWaypoint == 17)
                    {
                        std::cout << "[+] Moving to waypoints 17" << std::endl;
                        cv::Mat temp = cv::imread("target_17.jpeg");
                        rotate(temp, 270.0, d_img);
                    }

                    if(currentWaypoint == 18)
                    {
                        std::cout << "[+] Moving to waypoints 18" << std::endl;
                        cv::Mat temp = cv::imread("target_18.jpeg");
                        rotate(temp, 315.0, d_img);
                    }

                    // Change direction
                    if(currentWaypoint == 19)
                    {
                        std::cout << "[+] Moving to waypoints 19" << std::endl;
                        d_img = cv::imread("target_13.jpeg");
                        currentWaypoint = 0;
                    }
