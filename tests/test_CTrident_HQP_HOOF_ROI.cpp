...
int main(int argc, char *argv[])
{
    ...
    // 0, inf, 0 grad
//    CAdaptiveGain lambda_vx(1.666, 0.166, 1.666);
    ...
    char buffer [80];
    std::strftime(buffer,80,"../results/HOOF_HQP/%Y-%m-%d_%H:%M:%S.csv", now);
    ...
    cv::Mat mosaic = cv::imread("goal.jpg");
    cv::Mat mosaicPath = mosaic.clone();

    cvNamedWindow("Mosaic", CV_WINDOW_AUTOSIZE);

    utils::MouseEventNotifier mouse;
    utils::ClicksToPath clicks(mosaicPath);
    mouse.m_notifer.connect(boost::bind(&utils::IMouseObserver::notify, &clicks, _1, _2, _3, _4));
    cvSetMouseCallback("Mosaic", utils::mouseCallback, &mouse);
    cv::imshow("Mosaic", mosaicPath);

    //Wait for user to create path
    while(cv::waitKey() == -1);
    {
      cv::imshow("Mosaic", mosaicPath);
      cv::waitKey(30);
    ...
    std::vector<cv::Point> path = clicks.getPath();
...
*/
    ...
    */
    ...
    double width = 300.0;
    double height = 300.0;
    double x = path.at(0).x;
    double y = path.at(0).y;
    ...
    while(true)
    {
        ...
        // Set desired image
        double x_lower = x - (width/2.0);
        double y_lower = y - (height/2.0);
        ...
        if(x + (width/2.0) > mosaic.cols)
        {
            x_lower = mosaic.cols - width;
        }
        ...
        if(y + (height/2.0) > mosaic.rows)
        {
            y_lower = mosaic.rows - height;
        }

        cv::Rect ROIRect = cv::Rect(x_lower,y_lower,width,height);
        cv::Mat d_img = mosaic(ROIRect);

        dbg("")
        cv::imshow("Desired image", d_img);
        ...
        if(c_img.empty()==false)
        {
            ...
            rmatcher.match(c_img, d_img, matches, c_keypoints, d_keypoints);
            ...
            if(nMatches > 3)
            {
                ...
                trident.setDOFVelocities(cv::Mat(screw));
                ...
                if(speed < 0.05)
                {
                    ...
                    {
                        x = path.at(waypoint).x;
                        y = path.at(waypoint).y;
                    }
                    else
                    {
